...
contract YetiTest6 is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxCommunitySupply = 50;
...
\\   mapping(uint256 => bool) revealed;    
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    function presaleMint(uint256 _mintAmount) public payable {
        ...
        require(whitelist[msg.sender].isApproved, "You are not in the whitelist to mint a Yeti.");
        require(_mintAmount > 0, "Must at least mint 1 yeti");
        require(_mintAmount <= maxMintAmount, "Cannot mint more than 3 yetis at once");

        uint256 supply = totalSupply();
        require((supply + _mintAmount) < maxPresaleSupply, "Cannot exceed max number of presale tags");
        require(whitelist[msg.sender].reservedQuantity >= _mintAmount, "Insufficient reserved presale quantity");
        
        if (msg.sender != owner()) {
            require(msg.value >= cost * _mintAmount, "Ether value sent is not correct");
        }
        whitelist[msg.sender].reservedQuantity -= _mintAmount;

        for (uint i = 1;
            i <= _mintAmount;
            i++) {
            require((supply + i) < maxPresaleSupply, "Cannot exceed max presale supply");

            if (!_exists(supply + i)) {
                _safeMint(msg.sender, supply + i);
                revealTime[supply + i] = now;
            }
        }
    }
    ...
    returns (string memory)
  {
    ...
    uint memory _timeSpent = estimateRevealedTime(tokenId);
  ...
  function estimateRevealedTime(uint256 tokenId) public view returns (uint){
    uint current = now;
    return (current - revealTime[tokenId]) / 3600
  }
